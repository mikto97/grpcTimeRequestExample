...
This guide provides an overview of how to implement a simple scenario with grpc. We want to construct a scenario with two entities:
* A server
* A client
...
## Setting Up Server
NOTE: imports are not always explicitly mentioned (such as time, etc.). They should be added to the imports at the top.
...
NOTE: You will need to add `"google.golang.org/grpc"`to the imports section and then run `go mod tidy` for the above code to work (specifically to be able to use the NewServer method).
...
Copy this into the ``server.go`` file, turn it into a method, and change some things:
...
4. Implemented a method we specified in the proto file. 
...
Now we have to implement a method to connect to the server that can be used to obtain a connection, which can be used in the `waitForTimeRequest` function.
```
...
* Keep track of naming. For example, if the name of the service is changed to YearAsk then we e.g. also have to change the return statement in ``connectToServer`` to instead use `proto.NewYearAskClient`. This also goes for message and method changes.
